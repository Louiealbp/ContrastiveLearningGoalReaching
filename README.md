...
```bash mpirun -np 1 python -u train.py --env-name='FetchReach-v1' --n-cycles=10 2>&1 | tee reach.log
```
...
```
5. training on point_mass
'''
mpirun -np 8 python -u train.py --env-name='point_mass' --encoder_type='pixel' --frame_stack=1 2>&1 | tee pointmass.log
'''
